...
void __ISR(_DMA_1_VECTOR, IPL4SOFT) DMA0HANDLER(void)
{
    u32 i = 0;
    while (i < 32)
    {
...
void    init_DMA()
{
    ...
    IFS1bits.DMA1IF = 0;            // Clear any existing DMA channel 0 interrupt flag
    ...
    // Master Channel
    IPC9bits.DMA0IP = 4;            // Set Interrupt priority to 4
    IEC1bits.DMA0IE = 1;            // Enable DMA Channel 0 interrupts

    // Slave Channel
    IPC9bits.DMA1IP = 4;            // Set Interrupt priority to 4
    IPC9bits.DMA1IS = 2;            // Set Interrupt subpriority to 2
